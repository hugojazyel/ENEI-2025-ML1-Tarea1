...
- **$\eta=10^{-3}$**  
  ...
  - $\text{MSE}_{\text{test}}\approx \mathbf{0.571}$ y la distancia entre parámetros GD y OLS (||β_GD − β_OLS||_2) no es despreciable (soluciones alejadas).

- **$\eta=10^{-2}$**  
  - Convergencia **sustancialmente más rápida y estable**.  
  - $\text{MSE}_{\text{test}}\approx \mathbf{0.541}$, virtualmente igual a OLS; además además ||β_GD − β_OLS||_2 ≈ **0.0058** (parámetros casi idénticos).
